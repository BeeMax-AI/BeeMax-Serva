...
| 运营总览与趋势 | 指标、趋势、全库统计及业务报表；支持筛选，默认每页 10 条，可选 20 / 30 条；导出两页 PDF 运营简报 |
...
点击「导出简报」，按当前日 / 周 / 月范围下载 A4 两页 PDF：第一页展示核心指标、工单趋势及类型 / 小组分布，第二页展示重点工单与人员负载各前 5 条。期间新建、期间闭环与导出时的待办快照分别标注，附生成时间及统计说明。

中文字体内嵌，文字可复制，图表采用矢量绘制。部署需保留 `server/assets/fonts/` 的字体与许可证，以及 `design/gongdan-dashboard/assets/` 中的品牌图标；不依赖服务器系统字体。PDF 按登录客户生成，不在服务端归档。
...
- **白名单授权**：分别管理群和人员，记录 ID、名称及加入时间，默认每页 10 条，可选 20 / 30 条。
