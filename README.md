...
| 人员与配置 | 默认排班、今日当班、排班原文录入、路由规则、提醒参数、学习开关及渠道访问权限 |
| 企微账号 | 独立保存账号实例信息、配置主动推送群、查看配置变更记录 |
...
企微账号、主动推送群和 QiWe 凭据独立于业务 MCP 配置，即使 MCP 不可用也可管理。**当前已实现本地配置保存，尚未接入 QiWe 真实登录、在线检测、消息收发及推送执行**；保存成功不等于通道已连接，界面显示待连接、待同步或未验证。
